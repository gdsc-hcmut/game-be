...
import { Achievement, AchievementInfo, AchievementProgress, AchievementProgressDocument } from "../models/achievement.model";
...
export class AchievementService {
    ...
    async update(info: AchievementInfo): Promise<void> {
        ...
        let achievementProgress: AchievementProgressDocument = await AchievementProgress.findOne({ userId: info.userId });
        ...
        for (let achievement of achievementList) {
            if (achievement.type === info.achievementType) {
                if (!achievementProgress.achievements.some(x => x.id === achievement.id) &&
                    !(await achievement.handler(info, achievementProgress))) {
