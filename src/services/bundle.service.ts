...
// import { User, USER_FORBIDDEN_FIELDS, USER_SIMPLIFIED_FIELDS } from '../models/user.model';
...
//         const bundle = await this.bundleCollection.findOne({ _id: bundleId }) as Bundle;

//         // Validate 2 array
//         if (!_.isEqual(
//             _.sortBy((bundle.files as ObjectId[]).map((f: ObjectId) => f.toHexString())),
//             _.sortBy(files))) {
//             throw new Error('Mismatch files in bundle');
//         }

//         const affectedRow = await this.update(bundleId, { files: files.map(f => ObjectId.createFromHexString(f)) });
//         if (affectedRow == 0) throw new Error('Unable to update bundle files / Nothing changed');
...
//         const affectedRow = await this.update(createdBundleId, { slug: bundleSlug });
...
//         const opUpdateResult = await this.bundleCollection.updateOne({ _id: bundleId }, { $set: data });
//         return opUpdateResult.result.nModified;
//     }

//     async validateBundle(bundleId: ObjectId, userId: ObjectId) {
//         const bundle = await this.bundleCollection.findOne({ _id: bundleId, user: userId });
...
//         const affectedRow = await this.updateFile(insertedFile._id, { slug: fileSlug });
...
//         if (affectedBundle.modifiedCount == 0) throw new Error('Unable to add file to bundle');
...
//             {
//                 $push: {
//                     likes: { user: userId, createdAt: Math.floor(Date.now() / 1000), ip: clienIp },
//                 },
//             },
//         );

//         if (updateResult.result.nModified != 1) throw new Error('Bundle already liked');
//         return true;
//     }
...
//         if (updateResult.result.nModified != 1) throw new Error('Bundle not exist / already unliked');
//         return true;
//     }
...
//                 {
//                     $push: {
//                         views: { user: userId, createdAt: Math.floor(Date.now() / 1000), ip: clienIp },
//                     },
//                 },
//             );
//             return true;
...
//         const opUpdateResult = await this.fileCollection.updateOne({ _id: fileId }, { $set: data });
//         return opUpdateResult.result.nModified;
//     }

//     async find(query: any = {}, populateUser = false, limit = 10, simplify = false): Promise<Bundle[]> {
...
//         const bundles = await this.bundleCollection.aggregate(aggreateCommand).toArray();
...
//                         ({
//                             ..._.omit(f, 'isDeleted'),
//                             ...(simplify && { content: f.content.substring(0, 255) }),
//                         } as File),
//                 );
...
//         return ((await this.bundleCollection.countDocuments(query)) as any) as number;
//     }
...
//         if (opResult.result.nModified == 0) throw new Error('Unable to delete file in bundle');
